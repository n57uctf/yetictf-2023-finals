...
def push(args: PushArgs) -> CheckerResult:
    ...
    try:
        r = requests.post(f'{url}/register', json=creds_register_1)
    ...
    try:
        r = requests.post(f'{url}/register', json=creds_register_2)
        print(r.status_code, r.text)
        if r.status_code != 200:
            return CheckerResult(status=Status.MUMBLE.value,
                                 private_info=f'{r.status_code}',
    ...
    # Login
    try:
        r = requests.post(f'{url}/login', json=creds_login_1)
        print(r.status_code, r.text)
        if r.status_code != 200:
            return CheckerResult(status=Status.MUMBLE.value,
                                 private_info=f'{r.status_code}',
    ...
    # Login second user
    try:
        r = requests.post(f'{url}/login', json=creds_login_2)
        print(r.status_code, r.text)
        if r.status_code != 200:
            return CheckerResult(status=Status.MUMBLE.value,
                                 private_info=f'{r.status_code}',
